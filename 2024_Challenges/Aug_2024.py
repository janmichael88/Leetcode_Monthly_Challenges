...
class Solution:
    def countSubIslands(self, grid1: List[List[int]], grid2: List[List[int]]) -> int:
        '''
        better way would be to traverse every land cell in grid2, and as we traverse
        check whether this cell correspons to a land cell in grid1
        '''
        count = 0
        seen = set()
        rows = len(grid1)
        ...
        for i in range(rows):
            for j in range(cols):
                if grid2[i][j] == 1 and (i,j) not in seen:
                    if self.is_sub_island(grid1,grid2,i,j,rows,cols,seen):
                        count += 1
        return count
                
                
    def is_sub_island(self,g1,g2,i,j,rows,cols,seen):
        seen.add((i,j))
        dirrs = [[1,0],[-1,0],[0,1],[0,-1]]
        ans = g1[i][j] == 1
        for d_i,d_j in dirrs:
            ...
            if (0 <= ii < rows) and (0 <= jj < cols):
                if (ii,jj) not in seen and g2[ii][jj] == 1:
                    ...
                    #need to invoke dfs first and then && with the current (i,j)
                    ans = self.is_sub_island(g1,g2,ii,jj,rows,cols,seen) and ans
        
        return ans
    
#rewriting as BFS, or stack
class Solution:
    def countSubIslands(self, grid1: List[List[int]], grid2: List[List[int]]) -> int:
        '''
        for bfs, we just need to keep and'ing with all of a cells 
        '''
        count = 0
        seen = set()
        rows = len(grid1)
...
class Solution:
    def findingUsersActiveMinutes(self, logs: List[List[int]], k: int) -> List[int]:
        '''
        UAM for a given user is defined as the number of unique minutes in which the user peformed an action
        return array of size k, where array[i] is the number of users with UAM == i
        
        hashamp, user to unique minut actions
        '''
        user_to_unique = defaultdict(set)
        for user,minute in logs:
            user_to_unique[user].add(minute)
        
        #count of counts
        ans = [0]*k
        for user,unique in user_to_unique.items():
            count = len(unique)
            ans[count-1] += 1
